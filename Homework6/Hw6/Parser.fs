...
let isArgLengthSupported (args: string []) : Result<string [], Message> =
    ...
    | 3 -> Ok args
    | _ -> Error {Type= MessageType.WrongArgLength; MessageString = "WrongArgLength"}
...
let inline isOperationSupported (arg1, operation, arg2) : Result<'a * CalculatorOperation * 'b, Message> =
    ...
    | _ -> Error {Type= MessageType.WrongArgFormatOperation; MessageString = $"Could not parse value '{operation}'"}

let doubleTryparse (value: string) =
    match Double.TryParse(value, NumberStyles.Any, CultureInfo.InvariantCulture) with
    | true, arg -> Ok arg
    | _-> Error {Type= MessageType.WrongArgFormat; MessageString = $"Could not parse value '{value}'"}
...
let inline isDividingByZero (arg1, operation, arg2) =
    ...
    | CalculatorOperation.Divide, 0.0 -> Error {Type= MessageType.DivideByZero; MessageString = "DivideByZero"}
...
let parseCalcArguments (args: string []) =
    ...
    | Error message -> Error $"{message.MessageString}"